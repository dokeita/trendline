...
- [ ] `fetch_x/index.py`: ポスト保存時に `public_metrics`（`like_count`, `retweet_count`, `reply_count`, `impression_count`）を含める
- [ ] `summarize/index.py`: プロンプトにエンゲージメント指標を付記し、注目度の高い投稿を優先的に要約するよう指示を追加
